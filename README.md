...
| Ситуация | Поведение |
|---|---|
| Центр уходит за границу SVG | cx зажат в [0, 800] — эллипс может частично скрыться за краем |
...
> **Полная видимость эллипса при перетаскивании центра.**
> Сейчас `cx` зажат в `[0, 800]` — это предотвращает уход центра за пределы холста, но не гарантирует, что весь эллипс останется в видимой зоне (при `cx = 5` и `a = 100` левая часть уйдёт за левый край).
>
> Можно доработать, зажав `cx` в `[bbox_half, WIDTH − bbox_half]`, где `bbox_half = sqrt(a²cos²θ + b²sin²θ)` — это горизонтальный радиус габаритного прямоугольника эллипса. Тогда эллипс всегда будет полностью виден.
